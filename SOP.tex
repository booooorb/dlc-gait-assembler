...
The pipeline implements all 14 custom features: mean and variance of hindlimb
base of support, left and right paw-to-midline distance, bilateral phase offset,
bilateral stance overlap, and the mean height and vertical excursion of each
side-view MTP and knee marker. Height is calibrated per side view and measured
above the cycle-local lowest marker position so that image-coordinate direction
does not affect the result. It also derives six signed
contralesional-versus-ipsilesional asymmetry indices. Forelimb retraining remains
deferred, so this is a hindlimb-focused longitudinal analysis rather than a
complete forelimb-inclusive reproduction of Weber et al.
\end{tcolorbox}

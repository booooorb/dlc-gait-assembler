...
\text{epochs} =
\left\lceil
\frac{\text{target iterations}}
{
\left\lceil 
\frac{\text{labeled frames } \times \text{ training fraction}}
{\text{batch size}}
\right\rceil
}
\right\rceil
...
Note that the default batch size in DeepLabCut is 8, which can be edited in \texttt{config.yaml} at \texttt{Line 61}. It is not documented what batch size and training fraction are used for all papers, thus we assume a batch size of $B=8$ and a training fraction $f_{\text{train}}=0.75$ for epoch approximation: 
...
The 88 extracted ALMA hind limb parameters may not account for all relevant features in mouse gait analysis. ALMA only extracts the side-view hindlimb parameters, missing important gait features observable from different views and from the forelimbs. The \textbf{Hindlimb} option merges the existing 88 ALMA parameters with 30 non-overlapping RustLab1 parameters and 14 custom parameters, producing 132 features. The \textbf{Hindlimb + Forelimb} option adds 46 upstream RustLab1 forelimb/interlimb parameters, producing 178 features. To prevent overfitting, it is recommended to have a moderately large markerless-tracking dataset. Smoothing and gait cycle detection are shared across the merged outputs to standardize features.
...
\textbf{132 or 178 total features}
};
...
\noindent In the assembled pipeline, ALMA, RustLab1, custom, and synchronized
stroke outputs also share the configured kinematics likelihood threshold and
the same bidirectional linear interpolation before this filter is applied.
RustLab1 and custom calculations reuse that processed coordinate set rather
than applying a separate confidence cutoff or smoothing pass.
...
The stride start and end frames returned by ALMA are authoritative for every
post-ALMA feature table. RustLab1 and custom features are calculated inside
those exact windows. Synchronized bottom-view contact states may add phase,
overlap, and quality-control measurements, but they do not replace or shift
the ALMA stride boundaries.
...
\noindent The default \textbf{Hindlimb} mode retains 30 of the 106 RustLab1 outputs that are hindlimb-specific and non-overlapping with the 44 per-side ALMA parameters. The optional \textbf{Hindlimb + Forelimb} mode retains those 30 and adds 46 forelimb/interlimb outputs that ALMA does not provide. All RustLab1 calculations are evaluated within the shared ALMA gait-cycle windows so the tables can be merged by row. The default 30 parameters are as follows:

\begin{tcolorbox}[parameter table box,title=\textbf{RustLab1 Hindlimb Parameter List}]
\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{0.045\linewidth}
...
\begin{tcolorbox}[parameter table box,title=\textbf{Optional RustLab1 Forelimb/Interlimb Parameter List}]
\scriptsize
\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{0.04\linewidth}
...
\textbf{\#} & \textbf{Parameter Name} & \textbf{RustLab1 .csv Output Parameter Name} & \textbf{Units} \\
\boxedmidrule
1--3 & Left forepaw mean, maximum, and minimum bottom-view angle & \texttt{LF\_\_avg\_Angle}, \texttt{LF\_\_max\_Angle}, \texttt{LF\_\_min\_Angle} & deg \\
4--6 & Right forepaw mean, maximum, and minimum bottom-view angle & \texttt{RF\_\_avg\_Angle}, \texttt{RF\_\_max\_Angle}, \texttt{RF\_\_min\_Angle} & deg \\
7--8 & Left forepaw average height and vertical excursion & \texttt{l-front-toe-tip\_\_Average\_Height}, \texttt{l-front-toe-tip\_\_Movement} & mm \\
9--10 & Left wrist average height and vertical excursion & \texttt{l-wrist\_\_Average\_Height}, \texttt{l-wrist\_\_Movement} & mm \\
11--12 & Left elbow average height and vertical excursion & \texttt{l-elbow\_\_Average\_Height}, \texttt{l-elbow\_\_Movement} & mm \\
13--14 & Left shoulder average height and vertical excursion & \texttt{l-shoulder\_\_Average\_Height}, \texttt{l-shoulder\_\_Movement} & mm \\
15--16 & Right forepaw average height and vertical excursion & \texttt{r-front-toe-tip\_\_Average\_Height}, \texttt{r-front-toe-tip\_\_Movement} & mm \\
17--18 & Right wrist average height and vertical excursion & \texttt{r-wrist\_\_Average\_Height}, \texttt{r-wrist\_\_Movement} & mm \\
19--20 & Right elbow average height and vertical excursion & \texttt{r-elbow\_\_Average\_Height}, \texttt{r-elbow\_\_Movement} & mm \\
21--22 & Right shoulder average height and vertical excursion & \texttt{r-shoulder\_\_Average\_Height}, \texttt{r-shoulder\_\_Movement} & mm \\
23--26 & Left forepaw mean, median, protraction, and retraction relative to shoulder & \texttt{left\_\_front\_\_average}, \texttt{left\_\_front\_\_median}, \texttt{left\_\_front\_\_protraction}, \texttt{left\_\_front\_\_retraction} & mm \\
27--30 & Right forepaw mean, median, protraction, and retraction relative to shoulder & \texttt{right\_\_front\_\_average}, \texttt{right\_\_front\_\_median}, \texttt{right\_\_front\_\_protraction}, \texttt{right\_\_front\_\_retraction} & mm \\
31--33 & Left elbow mean, maximum, and minimum side-view angle & \texttt{avg\_Angle\_\_left\_\_front\_\_shoulder\_ellbow\_wrist}, corresponding \texttt{max\_Angle} and \texttt{min\_Angle} names & deg \\
34--36 & Left wrist mean, maximum, and minimum side-view angle & \texttt{avg\_Angle\_\_left\_\_front\_\_ellbow\_wrist\_toetip}, corresponding \texttt{max\_Angle} and \texttt{min\_Angle} names & deg \\
37--39 & Right elbow mean, maximum, and minimum side-view angle & \texttt{avg\_Angle\_\_right\_\_front\_\_shoulder\_ellbow\_wrist}, corresponding \texttt{max\_Angle} and \texttt{min\_Angle} names & deg \\
40--42 & Right wrist mean, maximum, and minimum side-view angle & \texttt{avg\_Angle\_\_right\_\_front\_\_ellbow\_wrist\_toetip}, corresponding \texttt{max\_Angle} and \texttt{min\_Angle} names & deg \\
43--44 & Left and right forelimb step duration & \texttt{left\_\_front\_\_seconds}, \texttt{right\_\_front\_\_seconds} & s \\
45 & Left-fore/right-hind asynchronous-phase fraction & \texttt{FLBR\_RATIO} & fraction \\
46 & Right-fore/left-hind asynchronous-phase fraction & \texttt{FRBL\_RATIO} & fraction \\
\boxedbottomrule
\end{tabularx}
\end{tcolorbox}
...
The final expanded gait dataset is created by merging ALMA, RustLab1, and custom post-ALMA features into one CSV file. Each row represents one gait cycle. The ALMA, RustLab1, and custom feature tables are matched using the shared gait-cycle row index. Hindlimb mode contains 132 features; Hindlimb + Forelimb mode contains 178:

\[
88\ \text{ALMA features}
+
30\text{ or }76\ \text{RustLab1 features}
+
14\ \text{custom features}
=
132\text{ or }178\ \text{total features}
\]
...
Features 89--118 (hindlimb) or 89--164 (combined) & RustLab1 & Post-ALMA hindlimb parameters plus the optional 46 forelimb/interlimb parameters \\
Features 119--132 (hindlimb) or 165--178 (combined) & Custom & Post-ALMA additional hindlimb features from custom scripts \\
\boxedbottomrule
\end{tabularx}
\end{tcolorbox}
...
The Gait Analysis window includes a separate \textbf{RustLab1 standalone
(three-view)} workflow. This option is independent of the ALMA gait-cycle
analysis. It requires one matched left-side, right-side, and bottom-view
DeepLabCut coordinate CSV for every trial. CSV pairing may be detected from the
file names or assigned manually, and the labels for every view must be checked
with \textbf{Label body parts}.

The selected bottom-view reference paw defines the stride cycles. RustLab1's
default rule classifies a frame as stance when the absolute frame-to-frame
horizontal paw displacement is no greater than $7$ pixels per frame. A stride
starts at a transition into stance and ends immediately before the next stance
onset. The interface also exposes minimum consecutive stance and swing lengths
to suppress short tracking artifacts. Candidate strides containing a paw speed
of $100$ pixels per frame or more are rejected by default, matching the
upstream tracking-jump filter. At least two stance onsets are required to
produce one complete stride, and the minimum number of accepted strides per
trial is configurable.

Before stride detection and parameter extraction, all three views use the same
configured confidence rejection, bidirectional interpolation, and Butterworth
low-pass coordinate preparation described above. The standalone defaults are a
minimum likelihood of $0.95$, a $6\,\mathrm{Hz}$ filter cutoff, and a stance
threshold of $7$ pixels per frame. These settings may be changed when the
recording frame rate or tracking characteristics require it. Changing any
setting invalidates the preview and requires a new
\textbf{RustLab1 stride preview}.

Side-view distances use the selected reference-body-segment or manual
pixel-to-centimetre calibration. The optional bottom-view horizontal
pixels-per-centimetre setting converts standalone stride length to centimetres;
when it is not supplied, bottom-view stride length remains available in pixels.

\begin{tcolorbox}[output file box,title=\textbf{Standalone RustLab1 Output Files}]
\begin{tabularx}{\linewidth}{
...
Output & Description \\
\boxedmidrule
\texttt{<trial>\_rustlab1\_strides.csv} & Reference-paw stance, swing, and stride boundaries with frame and time measurements. \\
\texttt{<trial>\_rustlab1\_parameters.csv} & Selected RustLab1 parameters calculated inside the standalone stride windows. \\
\texttt{<trial>\_rustlab1\_summary.csv} & Trial-level medians of the numeric RustLab1 stride parameters. \\
\texttt{<trial>\_rustlab1\_stride\_preview.svg} & Bottom-paw speed trace, stance threshold, and detected stride windows. \\
\texttt{<trial>\_rustlab1\_figures/} & Optional adapted 18-figure RustLab1 runway bundle. \\
\boxedbottomrule
\end{tabularx}
\end{tcolorbox}

This standalone option does not write ALMA parameter tables, custom SOP
parameters, merged ALMA/RustLab1 tables, or synchronized stroke-pilot outputs.
